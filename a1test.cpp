/* OOP244.132 Assignment 1 Test Main
...
*
*  Version 1.00
*  Version 1.01 - if (fp, 0, "10") == 0 didn't return "true", not "false"
*  Version 1.10 - isRegistered, isValid, check area, pub, title
*  Version 1.20 - Test valid (and invalid) ISBNs that end in X (MF)
*  Version 1.30 - Better error reporting for
*                   the case of invalid ISBN: XXXXXXXXX0   (MF)
*  Version 1.31 - Cleaned up older comments and some print messages (MF)
*  Version 1.32 - Test isValid before calling isRegistered (CS)
*
...
bool prefixTests(FILE* fp, int& testNumber, int* totalPassed,
 int* totalTested) {
    ...
    // test isRegistered(): Area version
	if(isRegistered(fp, -1) != 0) {
        failed("isRegistered(fp, -1) did not return false",
         ++testNumber, &ok, totalTested);
    } else
        passed("area isRegistered(fp, -1)", ++testNumber, totalPassed,
         totalTested);
    if(isRegistered(fp, 99955) != 0) {
        failed("area isRegistered(fp, 99955) did not return false",
         ++testNumber, &ok, totalTested);
    } else
        passed("area isRegistered(fp, 99955)", ++testNumber, totalPassed,
         totalTested);
    if(isRegistered(fp, 0) == 0) {
        failed("area isRegistered(fp, 0) did not return true", ++testNumber, &ok,
         totalTested);
    } else
        passed("area isRegistered(fp, 0)", ++testNumber, totalPassed,
         totalTested);

	// test isRegister(): Area Publisher
    if(isRegistered(fp, -1, "0") != 0) {
        failed("area-publisher isRegistered(fp, -1, \"0\") did not return false",
         ++testNumber, &ok, totalTested);
    } else
        passed("area-publisher isRegistered(fp, -1, \"0\")", ++testNumber, totalPassed,
         totalTested);
    if(isRegistered(fp, 99955, "0") != 0) {
        failed("area-publisher isRegistered(fp, 99955, \"0\") did not return false",
         ++testNumber, &ok, totalTested);
    } else
        passed("area-publisher isRegistered(fp, 99955, \"0\")", ++testNumber, totalPassed,
         totalTested);
    if(isRegistered(fp, 0, "10") == 0) {
        failed("area-publisher version isRegistered(fp, 0, \"10\") did not return true", ++testNumber,
         &ok, totalTested);
    } else
        passed("area-publisher isRegistered(fp, 0, \"10\")", ++testNumber, totalPassed,
         totalTested);
    if(isRegistered(fp, 99955, "0") != 0) {
        failed("area-publisher isRegistered(fp, 99955, \"0\") did not return false",
         ++testNumber, &ok, totalTested);
...
bool isbnPrefixTests(FILE* fp, int& testNumber, int* totalPassed,
 int* totalTested) {
    ...
    // test isRegistered for area-publisher-title
	if(isRegistered(fp, "9070002043", area, publisher, title) == 0) {
        failed("isRegistered(fp, \"9070002043\", ...) did not return true",
         ++testNumber, &ok, totalTested);
	} else {
	  if(strcmp(area,"90") == 0 && strcmp(publisher,"70002") == 0 && strcmp(title,"04") == 0) {
		  passed("isRegistered(fp, \"9070002043\", ...)", ++testNumber, totalPassed, totalTested);
	  } else {
    ...
    if(isRegistered(fp, "9995500000", area, publisher, title) != 0) {
        ...
        passed("isRegistered(fp, \"9995500000\", ...)", ++testNumber,
         totalPassed, totalTested);
    char message[51], value[][11] = {
    ...
    // calculate total_isbn using the sizeof() operator
    int total_isbn = sizeof(value)/sizeof(value[0]);
    printf("total_isbn %d\n", total_isbn);

    /* replaced by code that follows immediately Version 1.32
    for (int i = 0; i < total_isbn; i++) {
        if(isRegistered(fp, value[i], area, publisher, title) == 0) {
            if (strcmp("XXXXXXXXX0", value[i]) == 0){
              if (isValid(value[i]) == 0 ){
                  passed("", ++testNumber, totalPassed, totalTested);
              } else {
                  sprintf(message, "isValid(...) did not return false "
                    "for %s", value[i]);
                  failed(message, ++testNumber, &ok, totalTested);
              }
            } else {
              sprintf(message, "isRegistered(fp, ...) did not return true "
                  "for %s", value[i]);
              failed(message, ++testNumber, &ok, totalTested);
            }
          } else if (i%100 == 0) {
            sprintf(message, "isRegistered(fp, ...) returned true for %s",
             value[i]);
            passed(message, ++testNumber, totalPassed, totalTested);
        } else {
            passed("", ++testNumber, totalPassed, totalTested);
        }
        // ISBN's (except the last) ending in X are valid and registered.
        if (value[i][9] == 'X'){
          if (isValid(value[i])){
            if (isRegistered(fp, value[i], area, publisher, title) == 0) {
              sprintf(message, "isRegistered(fp, ...) did not return true "
          ...
          } else if (i < total_isbn - 1) {
              sprintf(message, "valid(\"%s\") returned true",
                value[i]);
              failed(message, ++testNumber, &ok, totalTested);
          } else {
            passed("", ++testNumber, totalPassed, totalTested);
    ...
    for (int i = 0; i < total_isbn; i++) {
        if (isValid(value[i])) {
            ...
            } else if(isRegistered(fp, value[i], area, publisher, title) == 0) {
                sprintf(message, "isRegistered(fp, ...) did not return true "
                    "for %s", value[i]);
                failed(message, ++testNumber, &ok, totalTested);
            } else if (i%100 == 0) {
            ...
            sprintf(message, "isValid(\"%s\") returned false",
                value[i]);
            passed(message, ++testNumber, totalPassed, totalTested);
        }
    }
